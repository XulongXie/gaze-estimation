    ...
    while (True):
        ...
        if (len(faces) != 0):
            ...
            for index, face in enumerate(faces):
                ...
                # right_up = (face.right(), face.top())
                # left_bottom = (face.left(), face.bottom())
                # 画面部检测框
                # cv2.rectangle(frame, right_up, left_bottom, (255, 255, 0), 2)
                # 面部特征点检测
                ...
                # 左眼做画圈处理
                for idx, point in enumerate(leftEye):
                    # 左眼的坐标
                    pos = (point[0], point[1])
                    cv2.circle(frame, pos, 2, (0, 255, 0), -1)
                # 右眼做画圈处理
                for idx, point in enumerate(rightEye):
                    # 右眼坐标
                    pos = (point[0], point[1])
                    cv2.circle(frame, pos, 2, (0, 255, 0), -1)
                # 左眼的眼角
                ...
                # 左眼眼角画框
                cv2.rectangle(frame, left_upper_left, left_down_right, (0, 0, 255), 2)
                # 右眼的眼角
                ...
                # 右眼眼角画框
                cv2.rectangle(frame, right_upper_left, right_down_right, (0, 0, 255), 2)
                '''
